...
public class IdBatchResolverTest  extends BatchTestCase {
    ...
    public void testNullMemberCheckChildren() throws Exception {
        ...
        final String sql =
          "select `time_by_day`.`the_year` as `c0` from `time_by_day` as `time_by_day` where UPPER(`time_by_day`.`the_year`) = UPPER('2018') group by `time_by_day`.`the_year` order by ISNULL(`time_by_day`.`the_year`) ASC, `time_by_day`.`the_year` ASC";
